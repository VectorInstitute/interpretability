...
get_multiclass_roc_auc_score, get_dist_auc = ultraimport(
    "__dir__/../utils/metrics.py",
    ["get_multiclass_roc_auc_score", "get_dist_auc"],
)
load_checkpoint, save_checkpoint = ultraimport(
    "__dir__/../utils/model_utils.py",
    ["load_checkpoint", "save_checkpoint"],
)
(visualize_trainable_attention,) = ultraimport(
    "__dir__/../utils/visualize.py", ["visualize_trainable_attention"]
)
cleanup, worker_init_fn, setup_distributed_training = ultraimport(
    "__dir__/../utils/dist_utils.py",
    ["cleanup", "worker_init_fn", "setup_distributed_training"],
)
XrayDataset, load_nih_data = ultraimport(
    "__dir__/../utils/nih_data_utils.py",
    ["XrayDataset", "load_nih_data"],
)


def get_data_loaders(
    cfg, train_ds: XrayDataset, test_ds: XrayDataset
...
def train_selfattention(
    ...
    for epoch in range(start_epoch + 1, cfg["train_params"]["num_epochs"]):
        ...
        y_tr_probs = np.empty((0, num_unique_classes), dtype=np.float32)
        y_tr_true = np.empty((0, num_unique_classes), dtype=np.float32)
        ...
        y_te_probs, y_te_true = eval_step(
            model, cfg, test_dl, epoch, device_id, visualize=True
        )
