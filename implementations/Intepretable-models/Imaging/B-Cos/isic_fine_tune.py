...
def get_optimizer(
    model: nn.Module, learning_rate: float, freeze_features: bool = False
) -> optim.Optimizer:
    """
...
def get_dataloaders(cfg) -> Tuple[DataLoader, DataLoader]:
    ...
    train_dataset = ISICDataset(
        root_dir=train_root, csv_file=train_csv, transform=transform
    )
    test_dataset = ISICDataset(
        root_dir=test_root, csv_file=test_csv, transform=transform
    )
...
def check_early_stopping(
    patience: int, best_loss: float, current_loss: float, counter: int
):
    """
...
def train_model_with_early_stopping(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        stop, best_loss, counter = check_early_stopping(
            patience, best_loss, test_loss, counter
        )
        if stop:
...
def main():
    ...
    model = get_model(
        num_classes=config["num_classes"], freeze_features=config["freeze_features"]
    )
    model = model.to(device)

    optimizer = get_optimizer(
        model, config["optimizer"]["learning_rate"], config["freeze_features"]
    )
    ...
    model_path = os.path.join(
        os.path.dirname(__file__), f'{config["output_dir"]}/{config["output_file"]}'
    )
