...
def train_model(
    ...
    hidden_units = config["train_params"][
        "hidden_units"
    ]  # Define the hidden units for each feature network
    coxnam_model = CoxNAM(
        num_features, input_dim, hidden_units, dropout_rate=dropout
    ).to(device)
...
def evaluate_model(
    ...
    with torch.no_grad():
        risk_scores_test = (
            coxnam_model(X_tensor).cpu().numpy().flatten()
...
def plot_shape_functions_and_distributions(
    model: torch.nn.Module, X: pd.DataFrame, feature_names: list
) -> None:
    """
...
def local_explanation(coxnam_model: torch.nn.Module, x_instance: torch.tensor) -> tuple:
    """
...
def main():
    ...
    plot_local_explanation_bar(
        coxnam_model, single_x, feature_names, sort_features=True
    )
