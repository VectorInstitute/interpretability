...
from coxnam.surv_utils import *  # noqa: E402, F403
from coxnam.plots import *  # noqa: E402, F403
...
def load_and_prepare_data(
    file_path: str = "../../datasets/framingham.csv", test_size: float = 0.2
) -> tuple:
    """
    ...
    event = df_imputed[
        "TenYearCHD"
    ].values  # Event indicator (1 = CHD occurred, 0 = censored)
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, num_samples, batch_size):
            ...
            if torch.isnan(loss):
                print(
                    f"NaN loss detected at epoch {epoch+1}, mini-batch {i//batch_size}"
                )
...
def evaluate_model(
    ...
    c_index = concordance_index(
        duration_test, -risk_scores_test, event_test
    )  # Use negative risk scores
...
def main():
    ...
    (
        ...
        X_test,
        df,
    ) = load_and_prepare_data(data_file)
    ...
    evaluate_model(
        coxnam_model,
        X_test_tensor,
        duration_test_tensor.numpy(),
        event_test_tensor.numpy(),
    )
    ...
    time_grid, H0, S0 = compute_baseline_survival(  # noqa: F405
        coxnam_model, X_train_tensor, duration_train_tensor, event_train_tensor
    )
    plot_baseline_survival(time_grid, S0)  # noqa: F405

    # Compute final survival probabilities for the test set
    survival_probs_test = compute_final_survival_probabilities(  # noqa: F405
        coxnam_model, X_test_tensor, time_grid, H0
    )
