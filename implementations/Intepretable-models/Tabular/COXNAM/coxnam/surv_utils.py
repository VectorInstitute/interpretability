...
def compute_baseline_survival(coxnam_model, X_train, durations_train, events_train, time_grid=None):
    """
    ...
    durations = durations_train.cpu().numpy().flatten() if isinstance(durations_train, torch.Tensor) else np.array(durations_train).flatten()
    events = events_train.cpu().numpy().flatten() if isinstance(events_train, torch.Tensor) else np.array(events_train).flatten()
