# extracted from https://github.com/ChantalMP/Xplainer?tab=readme-ov-file
...
from utils.zeroshot_utils import cos_sim_to_prob, log_prob_to_prob, prob_to_log_prob
...
class InferenceModel():
    ...
    def get_descriptor_probs(self, image_path, descriptors: List[str], do_negative_prompting=True, demo=False):
        probs = {}
        negative_probs = {}
    ...
    def get_predictions_bin_prompting(self, disease_descriptors, disease_probs, negative_disease_probs, keys):
        predicted_diseases = []
        prob_vector = torch.zeros(len(list(disease_descriptors.keys())), dtype=torch.float)  # num of diseases
        for idx, disease in enumerate(disease_descriptors):
            if disease == 'No Finding':
                continue
            pos_neg_scores = torch.tensor([disease_probs[disease], negative_disease_probs[disease]])
            
            prob_vector[list(disease_descriptors.keys()).index(disease)] = pos_neg_scores[0]
